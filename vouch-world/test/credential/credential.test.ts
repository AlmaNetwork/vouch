...
describe("typed credentials — varied, validated certificate types", () => {
  ...
  test("a variety of credential types each carry their own shape", () => {
    const certs = [
      issueCredential(MembershipCredential, { issuer: "guild@umi", subject: "alice@umi", claims: { org: "smiths", role: "master", since: "2024" }, issuedAt: ISSUED_AT }, issuer.privateKey),
      issueCredential(AssetCredential, { issuer: "bank@umi", subject: "alice@umi", claims: { kind: "land", amount: 3, unit: "plot" }, issuedAt: ISSUED_AT }, issuer.privateKey),
      issueCredential(EndorsementCredential, { issuer: "bob@umi", subject: "alice@umi", claims: { of: "alice@umi", weight: 4, note: "reliable" }, issuedAt: ISSUED_AT }, issuer.privateKey),
    ];
  ...
  test("a signed cert whose claims violate the type fails with 'invalid-claims'", () => {
    ...
    const cert = issueCertificate(
      { issuer: "guild@umi", subject: "alice@umi", schemaId: "alma.skill/v1", claims: { skill: "blacksmith", level: 99 }, issuedAt: ISSUED_AT },
      issuer.privateKey,
    );
    const res = verifyCredential(cert, issuer.publicKey, SkillCredential);
  ...
  test("verifying against the wrong type fails with 'schema-mismatch'", () => {
    const cert = issueCredential(SkillCredential, { issuer: "guild@umi", subject: "alice@umi", claims: { skill: "x", level: 1 }, issuedAt: ISSUED_AT }, issuer.privateKey);
  ...
  test("an unknown schemaId via the registry fails with 'unknown-credential-type'", () => {
    ...
    const cert = issueCredential(Unregistered, { issuer: "guild@umi", subject: "alice@umi", claims: { x: 1 }, issuedAt: ISSUED_AT }, issuer.privateKey);
  ...
  test("the endorsement type validates that 'of' is a real identifier", () => {
    expect(() =>
      issueCredential(EndorsementCredential, { issuer: "bob@umi", subject: "alice@umi", claims: { of: "not an id", weight: 3 }, issuedAt: ISSUED_AT }, issuer.privateKey),
    ).toThrow();
  });
  ...
  test("custom credential types compose with a registry", () => {
    ...
    const cert = issueCredential(LicenseCredential, { issuer: "gov@umi", subject: "alice@umi", claims: { scope: "trade", expiresAt: "2027-01-01" }, issuedAt: ISSUED_AT }, issuer.privateKey);
    const res = verifyCredentialWith(cert, issuer.publicKey, registry);
    expect(res.ok).toBe(true);
  ...
  test("credentials are deterministic under a seeded key + fixed issuedAt", () => {
    ...
    const a = issueCredential(SkillCredential, { issuer: "guild@umi", subject: "alice@umi", claims: { skill: "x", level: 3 }, issuedAt: ISSUED_AT }, k.privateKey);
    const b = issueCredential(SkillCredential, { issuer: "guild@umi", subject: "alice@umi", claims: { skill: "x", level: 3 }, issuedAt: ISSUED_AT }, k.privateKey);
