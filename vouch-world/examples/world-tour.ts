...
import { listAgents, currencySupply, getAgent } from "../src/agent";
import {
  amendInstitution,
  admitAgent,
  admitTreasury,
...
const lenient = () => makeInstitutions({ verificationPolicy: { acceptedSchemaIds: [], rejectUnknownSchemas: false }, diplomacyPolicy: { defaultStance: "absorb", overrides: {} } });
...
for (const name of ["ann", "bo", "cy"]) admitAgent(world, { id: `${name}@nova`, region: "nova", role: "merchant", valueProfile: "lenient", publicKey: "", currency: 100 });
...
log(`ann credit/reputation = ${getAgent(world.getState(), "ann@nova")?.balances.credit}/${getAgent(world.getState(), "ann@nova")?.reputation}`);

log("\n=== 4. vouch: ann vouches for bo (the brand verb -> trust) ===");
vouchFor(world, "ann@nova", "bo@nova", 3);
vouchFor(world, "cy@nova", "bo@nova", 2);
log(`bo trust = ${getAgent(world.getState(), "bo@nova")?.trust} (distinct from reputation ${getAgent(world.getState(), "bo@nova")?.reputation})`);
...
amendInstitution(world, "nova", { policy: "economy", value: { baseCostRate: 0.1, minCostRate: 0.05, repDiscount: 0.02, creditPerTx: 1 } }, "acct:alice");
amendInstitution(world, "nova", { policy: "governance", value: { kind: "council", members: ["acct:alice", "acct:dave"], threshold: 1 } }, "acct:alice");
...
log(`cross-border ann@nova -> zoe@umi: ${xb.ok ? `ok (fee ${xb.fee})` : `refused (${xb.reason})`}; zoe now has ${getAgent(world.getState(), "zoe@umi")?.balances.currency}`);
...
log(`sold: ${sale.ok}; nova owner=${nova?.owner} lifecycle=${nova?.lifecycle} governance=${nova?.institutions.governance.kind} (reset to new owner); residents still here: ${listAgents(world.getState()).filter((a) => a.region === "nova").length}`);
...
log(`metrics: tick=${m.tick} regions=${m.regions.total}(rec ${m.regions.recognized}) residents=${m.agents.residents} totalCurrency=${m.agents.totalCurrency} gini=${m.agents.currencyGini.toFixed(3)} logLen=${m.log.length} digest=${m.log.digest}`);
