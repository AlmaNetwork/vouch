...
import { parseInviteId, parseAccountId, parseResidentId } from "../../../domain/models/almaId.js";
  ...
  validate(payload: InvitePayload, ctx: CommandContext): void {
    ...
    if (!isNetworkFounded(state)) {
      throw new DomainError(
        "NETWORK_NOT_FOUNDED",
        "Cannot create invite: network not founded"
      );
    }
    ...
    if (payload.inviteId) {
      ...
      if (!parsed) {
        throw new DomainError(
          "VALIDATION_ERROR",
          `Invalid invite ID format: ${payload.inviteId}`,
      ...
      if (hasInvite(state, payload.inviteId as InviteId)) {
        throw new DomainError(
          "ALREADY_EXISTS",
          `Invite already exists: ${payload.inviteId}`,
          { inviteId: payload.inviteId }
        );
      }
    }
...
export const acceptInviteHandler: CommandHandler<"acceptInvite"> = {
  ...
  validate(payload: AcceptInvitePayload, ctx: CommandContext): void {
    ...
    if (!isNetworkFounded(state)) {
      throw new DomainError(
        "NETWORK_NOT_FOUNDED",
        "Cannot accept invite: network not founded"
      );
    }
    ...
    if (!invite) {
      throw new DomainError(
        "NOT_FOUND",
        `Invite not found: ${payload.inviteId}`,
        { inviteId: payload.inviteId }
      );
    }
    ...
    if (invite.status !== "pending") {
      throw new DomainError(
        "VALIDATION_ERROR",
        `Invite is not pending: ${payload.inviteId}`,
        { inviteId: payload.inviteId, status: invite.status }
      );
    }
    ...
    if (new Date(invite.expiresAt) < new Date(now)) {
      throw new DomainError(
        "VALIDATION_ERROR",
        `Invite has expired: ${payload.inviteId}`,
        { inviteId: payload.inviteId, expiresAt: invite.expiresAt }
      );
    }
...
export const suspendHandler: CommandHandler<"suspend"> = {
  ...
  validate(payload: SuspendPayload, ctx: CommandContext): void {
    ...
    if (!isNetworkFounded(state)) {
      throw new DomainError(
        "NETWORK_NOT_FOUNDED",
        "Cannot suspend account: network not founded"
      );
    }
    ...
    if (account.roles.includes("owner")) {
      throw new DomainError(
        "FORBIDDEN",
        "Cannot suspend the network owner"
      );
    }
    ...
    if (account.disabled) {
      throw new DomainError(
        "VALIDATION_ERROR",
        `Account already suspended: ${payload.accountId}`,
        { accountId: payload.accountId }
      );
    }
    ...
    if (!payload.reason || payload.reason.trim() === "") {
      throw new DomainError(
        "VALIDATION_ERROR",
        "Reason is required for suspending an account"
      );
    }
  },
...
export const reinstateHandler: CommandHandler<"reinstate"> = {
  ...
  validate(payload: ReinstatePayload, ctx: CommandContext): void {
    ...
    if (!isNetworkFounded(state)) {
      throw new DomainError(
        "NETWORK_NOT_FOUNDED",
        "Cannot reinstate account: network not founded"
      );
    }
    ...
    if (!account.disabled) {
      throw new DomainError(
        "VALIDATION_ERROR",
        `Account is not suspended: ${payload.accountId}`,
        { accountId: payload.accountId }
      );
    }
  },
